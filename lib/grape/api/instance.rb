    ...
    class Instance
      ...
      def collect_route_config_per_pattern(all_routes)
        ...
        routes_by_regexp.each_value do |routes|
          ...
          @router.suppress_synthesized_head_for(last_route.pattern_regexp) if routes.size > 1
          allowed_methods = routes.size > 1 ? [routes.first.request_method] : routes.map(&:request_method)
